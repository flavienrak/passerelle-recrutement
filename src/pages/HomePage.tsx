      ...
      <Card className="overflow-hidden">
            ...
            <thead>
              <tr className="bg-gradient-to-r from-[#1A1E2E] to-[#292E3E] border-b border-gray-700/50">
                {['topicOwner', 'candidateName', 'email'].map((field) => (
                  <th key={field} className="text-left p-4 first:rounded-tl-lg">
                    <div className="space-y-2">
                      <div className="text-[#FF6B00] font-medium">
                        {field === 'topicOwner' && 'Topic owner'}
                        {field === 'candidateName' && 'Nom du candidat'}
                        {field === 'email' && 'Email'}
                      </div>
                      <input
                        type="text"
                ...
                <th className="text-left p-4 text-[#FF6B00] font-medium">
                  <div className="space-y-2">
                    <div className="text-[#FF6B00] font-medium">Report</div>
                ...
                {[
                  ...
                  <th key={field} className="text-left p-4">
                    <div className="space-y-2">
                      <div className="text-[#FF6B00] font-medium">
                        {field === 'targetPosition' && 'Poste visé'}
                        {field === 'location' && 'Localisation'}
                        {field === 'targetCompany' && 'Entreprise cible'}
                        {field === 'status' && 'Statut'}
              ...
              {(currentData as Candidate[]).map((candidate, index) => (
                <tr
                  ...
                  className={`border-b border-gray-700/50 last:border-0 transition-colors duration-200 ${
                    index % 2 === 0 ? 'bg-[#1A1E2E]/20' : 'bg-transparent'
                  } hover:bg-[#1A1E2E]/40`}
                >
                  {(
                    [
                    ...
                    >
                      ...
                      editingCell?.field === field ? (
                        <input
                          ...
                          className="w-full bg-gray-800 border border-[#FF6B00] rounded px-2 py-1.5 text-white focus:ring-2 focus:ring-[#FF6B00]/30 focus:outline-none"
                          value={
                            field === 'dateAdded' && candidate[field]
                              ? String(candidate[field]).split('T')[0]
                              : candidate[field] || ''
                          }
                          onChange={(e) =>
                            handleCellEdit(index, field, e.target.value)
        ...
        <div className="flex items-center justify-between border-t border-gray-700/50 px-4 py-3 bg-gradient-to-r from-[#1A1E2E] to-[#292E3E]">
          <div className="flex items-center text-sm text-gray-400">
            <span>
              Affichage {startIndex + 1}-{Math.min(endIndex, activeData.length)}{' '}
              sur {activeData.length}
            </span>
          </div>
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#0A0E17] to-[#1A1E2E] flex">
      ...
      <div className="flex-1 p-8">
        {activeSection === 'candidates' ? (
          ...
          <div className="max-w-5xl mx-auto">
            <div className="flex justify-between items-center mb-8">
              <h1 className="text-3xl font-bold text-white">Espace Client</h1>
            ...
            <Card className="overflow-hidden">
              <div className="overflow-x-auto">
                <table className="w-full">
                      ...
                      {[
                        ...
                        >
                          <div className="space-y-2">
                            <div className="text-[#FF6B00] font-medium">
                              {field === 'topicOwner' && 'Topic owner'}
                              {field === 'interlocuteurRH' &&
                                'Interlocuteur RH'}
                              {field === 'company' && 'Entreprise'}
                              {field === 'candidatEnvoye' && 'Candidat envoyé'}
                              {field === 'poste' && 'Poste'}
                              {field === 'zoneGeo' && 'Zone géo'}
                              {field === 'commentaire' && 'Commentaire'}
                              {field === 'date' && "Date d'envoi"}
                            </div>
                            <input
                              type="text"
                              value={filters[field as keyof Report] || ''}
                              onChange={(e) =>
                                setFilters((prev) => ({
                                  ...prev,
                                  [field]: e.target.value,
                  ...
                  <tbody>
                    {(currentData as Report[]).map((report, index) => (
                      <tr
                        key={index}
                        className={`border-b border-gray-700/50 last:border-0 transition-colors duration-200 ${
                          index % 2 === 0 ? 'bg-[#1A1E2E]/20' : 'bg-transparent'
                      ...
                      >
                        ...
                        >
                          {editingCell?.row === index &&
                          editingCell?.field === 'topicOwner' ? (
                            <input
                              type="text"
                              className="w-full bg-gray-800 border border-[#FF6B00] rounded px-2 py-1.5 text-white focus:ring-2 focus:ring-[#FF6B00]/30 focus:outline-none"
                              value={report.topicOwner || ''}
                              onChange={(e) =>
                                handleCellEdit(
                                  index,
                                  'topicOwner',
                        ...
                        {(
                          ...
                          >
                            ...
                            editingCell?.field === field ? (
                              ...
                              >
                                {field === 'date'
                                  ? new Date(report[field]).toLocaleDateString(
                                      'fr-FR'
                                    )
                                  : report[field] || (
                                      <span className="text-gray-500 italic">
          ...
          <div className="max-w-4xl mx-auto">
            <div className="text-center mb-12">
              ...
              <p className="text-xl text-gray-300 mb-8">
                Boostez votre employabilité grâce à l'analyse cognitive Les
                recruteurs
              </p>
            ...
            <div className="grid md:grid-cols-3 gap-6">
              <div className="bg-[#1A1E2E]/80 backdrop-blur-sm border border-gray-700/50 rounded-xl p-6">
                ...
                <h2 className="text-xl font-bold text-white mb-2">Candidats</h2>
                <p className="text-gray-300">
                  Découvrez vos forces cognitives et maximisez vos chances en
                  entretien.
                </p>
              </div>
              ...
              <div className="bg-[#1A1E2E]/80 backdrop-blur-sm border border-gray-700/50 rounded-xl p-6">
                ...
                <h2 className="text-xl font-bold text-white mb-2">Clients</h2>
                <p className="text-gray-300">
                  Optimisez vos recrutements grâce à notre analyse cognitive
                  avancée.
                </p>
              </div>
              ...
              <div className="bg-[#1A1E2E]/80 backdrop-blur-sm border border-gray-700/50 rounded-xl p-6">
                ...
                <h2 className="text-xl font-bold text-white mb-2">À propos</h2>
                <p className="text-gray-300">
